...
class Mask2FormerDecoder(nn.Module):
    ...
    def forward(self, features: List[torch.Tensor]):
        ...
        for i, (mlp_layer, attn_layer) in enumerate(zip(self.mlp_layers, self.attention_layers)):
            ...
            # Downsample to reduce spatial dimensions before attention (memory optimization)
            # Reduces from potentially 256x256 (65k tokens) to 16x16 (256 tokens)
            pooled_size = 16
            feature_pooled = F.adaptive_avg_pool2d(feature, (pooled_size, pooled_size))

            # Apply self-attention on downsampled features
            feat_attn = feature_pooled.flatten(2).permute(0, 2, 1)  # [B, pooled_size*pooled_size, C]
            attn_output, _ = attn_layer(feat_attn, feat_attn, feat_attn)  # Self-attention on 256 tokens instead of 65k
            feat_attn = attn_output.permute(0, 2, 1).reshape(batch, channels, pooled_size, pooled_size)

            # Upsample attention output back to original spatial size
            feat_attn = F.interpolate(feat_attn, size=(height, width), mode="bilinear", align_corners=False)

            # Add residual connection and use attended features
            feature = feature + feat_attn  # Residual connection
